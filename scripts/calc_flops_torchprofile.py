"""
使用 torchprofile 库计算 Restormer 的参数量和 FLOPs
"""
import sys
import os
...
def calculate_flops_torchprofile(input_size=(1, 3, 128, 128)):
    """使用 torchprofile 计算 FLOPs 和参数量
    
    Args:
        input_size: 输入张量的尺寸，格式为 (batch_size, channels, height, width)
    """
    print("=" * 60)
    print("使用 torchprofile 库计算 Restormer 的参数量和 FLOPs")
    print("=" * 60)
...
if __name__ == "__main__":
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='使用 torchprofile 计算 Restormer 的参数量和 FLOPs')
    ...
    result = calculate_flops_torchprofile(input_size)
    
    if result:
        # 保存结果到文件
        output_file = os.path.join(os.path.dirname(__file__), "results_torchprofile.txt")
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write("=" * 60 + "\n")
            f.write("torchprofile 计算结果\n")
            f.write("=" * 60 + "\n")
