...
class FirstBlockVisualizer(nn.Module):
    ...
    def forward(self, x):
        ...
        # LayerNorm + FFN
        normed2 = self.norm2(first_residual)
        ...
        result = {
            ...
            "first_residual": first_residual,   # 第一次残差连接后的结果
            "ffn_output": ffn_out,              # FFN输出
            "second_residual": second_residual, # 第二次残差连接后的结果
        }
    ...
    def load_from_checkpoint(self, checkpoint_path, device):
        ...
        # LayerNorm 2
        norm2_dict = {k.replace(f"{block_prefix}norm2.", ""): v 
                      for k, v in state_dict.items() if k.startswith(f"{block_prefix}norm2.")}
        if norm2_dict:
            self.norm2.load_state_dict(norm2_dict, strict=False)
            print(f"Loaded: encoder_level1.0.norm2")
        
        # FFN
        ffn_dict = {k.replace(f"{block_prefix}ffn.", ""): v 
                    for k, v in state_dict.items() if k.startswith(f"{block_prefix}ffn.")}
        if ffn_dict:
            self.ffn.load_state_dict(ffn_dict, strict=False)
            print(f"Loaded: encoder_level1.0.ffn")
...
def visualize_all(input_img, outputs, attn_type, save_dir):
    ...
    # 定义网络流程顺序的数字前缀
    # 1: Input, 2: Patch Embedding, 3: Block Input, 4: Q/K/V (同时产生)
    # 5: Attention Map, 6: Attention*V, 7: Attention Output
    # 8: First Residual, 9: FFN Output, 10: Second Residual
    ...
    first_residual = outputs["first_residual"]
    ffn_output = outputs["ffn_output"]
    second_residual = outputs["second_residual"]
    
    # 创建大图: 3行4列（增加一行显示残差连接结果）
    ...
    im2 = ax2.imshow(patch_l2, cmap="jet")
    ...
    im3 = ax3.imshow(block_l2, cmap="jet")
    ...
    im4 = ax4.imshow(q_np, cmap="jet", aspect="auto")
    ...
    im5 = ax5.imshow(k_np, cmap="jet", aspect="auto")
    ...
    im6 = ax6.imshow(v_np, cmap="jet", aspect="auto")
    ...
    im7 = ax7.imshow(attn_np, cmap="jet", aspect="auto")
    ...
    im8 = ax8.imshow(attn_v_l2, cmap="jet")
    ...
    # Row 3: Attention Output, First Residual, FFN Output, Second Residual
    ax9 = plt.subplot(3, 4, 9)
    attn_out_l2 = compute_l2_norm(attn_output)
    im9 = ax9.imshow(attn_out_l2, cmap="jet")
    ax9.set_title(f"Attention Output (L2 Norm)\n{list(attn_output.shape)}", fontsize=10)
    ax9.axis("off")
    plt.colorbar(im9, ax=ax9, fraction=0.046, pad=0.04)
    
    ax10 = plt.subplot(3, 4, 10)
    first_res_l2 = compute_l2_norm(first_residual)
    im10 = ax10.imshow(first_res_l2, cmap="jet")
    ax10.set_title(f"First Residual (L2 Norm)\nInput + Attn\n{list(first_residual.shape)}", fontsize=10)
    ax10.axis("off")
    plt.colorbar(im10, ax=ax10, fraction=0.046, pad=0.04)
    
    ax11 = plt.subplot(3, 4, 11)
    ffn_l2 = compute_l2_norm(ffn_output)
    im11 = ax11.imshow(ffn_l2, cmap="jet")
    ax11.set_title(f"FFN Output (L2 Norm)\n{list(ffn_output.shape)}", fontsize=10)
    ax11.axis("off")
    plt.colorbar(im11, ax=ax11, fraction=0.046, pad=0.04)
    
    ax12 = plt.subplot(3, 4, 12)
    second_res_l2 = compute_l2_norm(second_residual)
    im12 = ax12.imshow(second_res_l2, cmap="jet")
    ax12.set_title(f"Second Residual (L2 Norm)\nFirst Res + FFN\n{list(second_residual.shape)}", fontsize=10)
    ax12.axis("off")
    plt.colorbar(im12, ax=ax12, fraction=0.046, pad=0.04)
    
    plt.suptitle(f"{attn_type} - Encoder Level 1 Block 0 Visualization (with Residuals)", fontsize=14, y=0.98)
    plt.tight_layout()
    
    save_path = os.path.join(save_dir, f"0_{attn_type}_complete_visualization.png")
    ...
    save_path_simple = os.path.join(save_dir_simple, f"0_{attn_type}_complete_visualization.png")
    plt.savefig(save_path_simple, dpi=150, bbox_inches="tight", pad_inches=0)
    plt.close()
    ...
    im = ax.imshow(attn_v_before, cmap="jet", aspect="auto")
    ...
    save_path = os.path.join(save_dir, f"6_{attn_type}_attn_v_before_rearrange.png")
    plt.savefig(save_path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {save_path}")
    ...
    save_path_simple = os.path.join(save_dir_simple, f"6_{attn_type}_attn_v_before_rearrange.png")
    plt.savefig(save_path_simple, dpi=150, bbox_inches="tight", pad_inches=0)
    plt.close()
    print(f"Saved (simple): {save_path_simple}")
    ...
    im = ax.imshow(attn_v_after_l2, cmap="jet")
    ax.set_title(f"{attn_type} - Attention*V (L2 Norm)\nAFTER rearrange BEFORE project_out\n{list(attn_v_after_rearrange.shape)}", fontsize=11)
    ax.axis("off")
    plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    plt.tight_layout()
    
    save_path = os.path.join(save_dir, f"6_{attn_type}_attn_v_after_rearrange.png")
    plt.savefig(save_path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {save_path}")
    ...
    save_path_simple = os.path.join(save_dir_simple, f"6_{attn_type}_attn_v_after_rearrange.png")
    plt.savefig(save_path_simple, dpi=150, bbox_inches="tight", pad_inches=0)
    plt.close()
    print(f"Saved (simple): {save_path_simple}")
    ...
    fig, ax = plt.subplots(figsize=(8, 6))
    attn_out_l2 = compute_l2_norm(attn_output)
    im = ax.imshow(attn_out_l2, cmap="jet")
    ax.set_title(f"{attn_type} - Attention Output (L2 Norm)\nAFTER project_out\n{list(attn_output.shape)}", fontsize=11)
    ax.axis("off")
    plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    plt.tight_layout()
    
    save_path = os.path.join(save_dir, f"7_{attn_type}_attn_output_after_proj.png")
    plt.savefig(save_path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {save_path}")
    ...
    save_path_simple = os.path.join(save_dir_simple, f"7_{attn_type}_attn_output_after_proj.png")
    plt.savefig(save_path_simple, dpi=150, bbox_inches="tight", pad_inches=0)
    plt.close()
    print(f"Saved (simple): {save_path_simple}")
    
    # 额外保存 First Residual (Input + Attention)
    fig, ax = plt.subplots(figsize=(8, 6))
    first_res_l2 = compute_l2_norm(first_residual)
    im = ax.imshow(first_res_l2, cmap="jet")
    ax.set_title(f"{attn_type} - First Residual (L2 Norm)\nInput + Attention\n{list(first_residual.shape)}", fontsize=11)
    ax.axis("off")
    plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    plt.tight_layout()
    
    save_path = os.path.join(save_dir, f"8_{attn_type}_first_residual.png")
    plt.savefig(save_path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {save_path}")
    ...
    save_path_simple = os.path.join(save_dir_simple, f"8_{attn_type}_first_residual.png")
    plt.savefig(save_path_simple, dpi=150, bbox_inches="tight", pad_inches=0)
    plt.close()
    print(f"Saved (simple): {save_path_simple}")
    ...
    fig, ax = plt.subplots(figsize=(8, 6))
    ffn_l2 = compute_l2_norm(ffn_output)
    im = ax.imshow(ffn_l2, cmap="jet")
    ax.set_title(f"{attn_type} - FFN Output (L2 Norm)\n{list(ffn_output.shape)}", fontsize=11)
    ax.axis("off")
    plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    plt.tight_layout()
    
    save_path = os.path.join(save_dir, f"9_{attn_type}_ffn_output.png")
    plt.savefig(save_path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {save_path}")
    ...
    save_path_simple = os.path.join(save_dir_simple, f"9_{attn_type}_ffn_output.png")
    plt.savefig(save_path_simple, dpi=150, bbox_inches="tight", pad_inches=0)
    plt.close()
    print(f"Saved (simple): {save_path_simple}")
    
    # 额外保存 Second Residual (First Residual + FFN)
    fig, ax = plt.subplots(figsize=(8, 6))
    second_res_l2 = compute_l2_norm(second_residual)
    im = ax.imshow(second_res_l2, cmap="jet")
    ax.set_title(f"{attn_type} - Second Residual (L2 Norm)\nFirst Residual + FFN\n{list(second_residual.shape)}", fontsize=11)
    ax.axis("off")
    plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    plt.tight_layout()
    
    save_path = os.path.join(save_dir, f"10_{attn_type}_second_residual.png")
    plt.savefig(save_path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {save_path}")
    ...
    save_path_simple = os.path.join(save_dir_simple, f"10_{attn_type}_second_residual.png")
    plt.savefig(save_path_simple, dpi=150, bbox_inches="tight", pad_inches=0)
    plt.close()
    print(f"Saved (simple): {save_path_simple}")
...
def save_individual_maps(outputs, attn_type, save_dir):
    ...
    for name, tensor in maps_before.items():
        ...
        save_path = os.path.join(save_dir, f"4_{attn_type}_{name}.png")
        ...
        save_path_simple = os.path.join(save_dir_simple, f"4_{attn_type}_{name}.png")
        plt.savefig(save_path_simple, dpi=200, bbox_inches="tight", pad_inches=0)
        plt.close()
        print(f"Saved (simple): {save_path_simple}")
    
    # 保存 Q, K, V, Attention Map (after rearrange - 用于计算注意力)
    # 数字前缀: 4 (Q/K/V after rearrange), 5 (Attention Map)
    maps_after = {
    ...
    for name, data in maps_after.items():
        ...
        save_path = os.path.join(save_dir, f"{prefix}_{attn_type}_{name}.png")
        plt.savefig(save_path, dpi=200, bbox_inches="tight")
        plt.close()
        print(f"Saved: {save_path}")
        ...
        save_path_simple = os.path.join(save_dir_simple, f"{prefix}_{attn_type}_{name}.png")
        plt.savefig(save_path_simple, dpi=200, bbox_inches="tight", pad_inches=0)
        plt.close()
        print(f"Saved (simple): {save_path_simple}")
