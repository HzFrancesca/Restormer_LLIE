...
print("=" * 80)
print("torchprofile测试: 验证返回MACs还是FLOPs")
print("=" * 80)
...
# 使用torchprofile统计
print(f"\n{'=' * 80}")
print("torchprofile统计结果:")
print("=" * 80)

macs = profile_macs(model_with_bias, input_data)

print(f"\ntorchprofile报告的MACs: {macs:,}")
...
if macs == total_macs:
    print(f"\n✓ 结论: torchprofile返回的是 MACs")
    print(f"  验证: {macs:,} == {total_macs:,}")
elif macs == total_flops:
    print(f"\n✓ 结论: torchprofile返回的是 FLOPs")
    print(f"  验证: {macs:,} == {total_flops:,}")
elif macs == matmul_macs:
    print(f"\n✓ 结论: torchprofile返回的是 MACs (仅矩阵乘法，不含bias)")
    print(f"  验证: {macs:,} == {matmul_macs:,}")
elif macs == matmul_flops:
    print(f"\n✓ 结论: torchprofile返回的是 FLOPs (仅矩阵乘法，不含bias)")
    print(f"  验证: {macs:,} == {matmul_flops:,}")
elif macs == matmul_macs + bias_flops:
    print(f"\n✓ 结论: torchprofile返回的是 MACs + bias加法")
    print(f"  验证: {macs:,} == {matmul_macs:,} + {bias_flops:,}")
else:
    print(f"\n✗ 警告: torchprofile的值不匹配预期")
    print(f"  差异: {macs} vs MACs:{total_macs} FLOPs:{total_flops}")
...
macs_no_bias = profile_macs(model_no_bias, input_data)

print(f"\n无bias时:")
print(f"  理论MACs:  {matmul_macs:,}")
print(f"  理论FLOPs: {matmul_flops:,}")
print(f"  torchprofile: {macs_no_bias:,}")

if macs_no_bias == matmul_macs:
    print(f"  ✓ 匹配: torchprofile = 理论MACs")
elif macs_no_bias == matmul_flops:
    print(f"  ✓ 匹配: torchprofile = 理论FLOPs")
else:
    print(f"  ✗ 不匹配")
...
if macs == total_macs + bias_flops:
    print(f"  ✓ 可能: torchprofile = MACs + bias加法")
...
if macs_no_bias == matmul_macs and macs == matmul_macs:
    print("torchprofile返回: MACs (仅矩阵乘法，不计bias)")
    ...
    print("torchprofile返回: FLOPs (仅矩阵乘法，不计bias)")
